...
if __name__ == "__main__":
    ...
    # create evaluator
    class_evaluator = PanopticEval(
        nr_classes, None, ignore_class, min_points=FLAGS.min_inst_points
    )
    ...
    for sequence in test_sequences:
        ...
        seq_label_names = sorted(
            [
                os.path.join(label_paths, fn)
                for fn in os.listdir(label_paths)
    ...
    for sequence in test_sequences:
        ...
        seq_pred_names = sorted(
            [
                os.path.join(pred_paths, fn)
                for fn in os.listdir(pred_paths)
                if fn.endswith(".label")
            ]
        )
    ...
    for label_file, pred_file in zip(label_names, pred_names):
        ...
        class_evaluator.addBatch(
            u_pred_sem_class, u_pred_inst, u_label_sem_class, u_label_inst
        )
    ...
    PQ_dagger = np.mean(
        [float(output_dict[c]["PQ"]) for c in things]
        + [float(output_dict[c]["IoU"]) for c in stuff]
    )
