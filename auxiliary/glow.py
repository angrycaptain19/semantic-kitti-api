...
class GlTextureRectangle:
    def __init__(self, width, height, internalFormat=gl.GL_RGBA, format=gl.GL_RGBA):
        ...
        self.internalFormat_ = (
            internalFormat  # gl.GL_RGB_FLOAT, gl.GL_RGB_UNSIGNED, ...
        )
        ...
        gl.glTexParameteri(
            gl.GL_TEXTURE_RECTANGLE, gl.GL_TEXTURE_MIN_FILTER, gl.GL_NEAREST
        )
        gl.glTexParameteri(
            gl.GL_TEXTURE_RECTANGLE, gl.GL_TEXTURE_MAG_FILTER, gl.GL_NEAREST
        )
        gl.glTexParameteri(
            gl.GL_TEXTURE_RECTANGLE, gl.GL_TEXTURE_WRAP_S, gl.GL_CLAMP_TO_BORDER
        )
        gl.glTexParameteri(
            gl.GL_TEXTURE_RECTANGLE, gl.GL_TEXTURE_WRAP_T, gl.GL_CLAMP_TO_BORDER
        )
...
class GlProgram:
    ...
    def __setitem__(self, name, value):
        ...
        if name not in self.uniform_types_:
            if WARN_INVALID_UNIFORMS:
                print("No uniform with name '{}' available.".format(name))
            return
    ...
    def link(self):
        if (
            gl.GL_VERTEX_SHADER not in self.shaders_
            or gl.GL_FRAGMENT_SHADER not in self.shaders_
        ):
